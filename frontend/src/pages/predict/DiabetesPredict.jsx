...
const DiabetesPredict = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setResult(res.data);
      // Auto-save prediction to MongoDB history
      try {
      ...
      setError('Failed to analyze diabetes risk. Please check server connection.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Diabetes Risk Diagnostic Engine</h1>
          <p className="text-xs text-slate-400">Input patient metabolic indicators for ML evaluation & SHAP feature analysis</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-6 glass-card p-6 rounded-3xl border border-slate-800">
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Pregnancies</label>
                <input
                  type="number"
                  name="pregnancies"
                  min="0"
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Glucose (mg/dL)</label>
                <input
                  type="number"
                  name="glucose"
                  min="0"
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Skin Thickness (mm)</label>
                <input
                  type="number"
                  name="skin_thickness"
                  min="0"
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Insulin (mu U/ml)</label>
                <input
                  type="number"
                  name="insulin"
                  min="0"
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">BMI (kg/m²)</label>
                <input
                  type="number"
                  step="0.1"
                  name="bmi"
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Pedigree Function</label>
                <input
                  type="number"
                  step="0.001"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-4 py-3 px-4 rounded-xl bg-gradient-to-r from-amber-500 to-orange-600 text-white font-semibold text-sm shadow-lg shadow-amber-500/20 hover:scale-[1.01] transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
            >
              {loading ? (
        ...
        <div className="lg:col-span-6 space-y-6">
          {result ? (
              ...
              <div className={`p-6 rounded-3xl border ${result.status === 'Positive' ? 'bg-rose-500/10 border-rose-500/30 text-rose-200' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-200'}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-xs uppercase tracking-wider font-semibold opacity-75">Diagnostic Result</span>
                      <h2 className="text-2xl font-extrabold text-white">
                        {result.status === 'Positive' ? 'HIGH DIABETES RISK DETECTED' : 'LOW DIABETES RISK'}
                      </h2>
                    </div>
                  </div>
            ...
            <div className="glass-card p-12 rounded-3xl border border-slate-800 text-center flex flex-col items-center justify-center h-full text-slate-400">
              ...
              <p className="text-xs text-slate-500 max-w-xs mt-1">Fill out the clinical form and click "Run Diabetes Risk Analysis" to generate ML predictions and SHAP explainability insights.</p>
            </div>
          )}
        </div>
      </div>
