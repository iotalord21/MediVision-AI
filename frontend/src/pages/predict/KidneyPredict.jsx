...
const KidneyPredict = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData({
      ...formData,
      [name]: name === 'rbc' || name === 'pc' || name === 'htn' || name === 'dm' ? value : (parseFloat(value) || 0)
    });
    setError('');
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 transition-colors duration-300">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{t('predict.kidneyTitle')}</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">{t('predict.kidneySubtitle')}</p>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {loading ? (
                ...
                `${t('common.predict')} (${t('diseases.kidney.name')})`
              )}
            </button>
          </form>
        </div>
