  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 transition-colors duration-300">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{t('predict.heartTitle')}</h1>
          <p className="text-xs text-slate-500 dark:text-slate-400">{t('predict.heartSubtitle')}</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-6 glass-card p-6 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-6 self-start">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">{t('predict.labels.sex')}</label>
                <select
                  name="sex"
                  value={formData.sex}
                  onChange={handleChange}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">{t('predict.labels.restingBp')}</label>
                <input
                  type="number"
                  name="trestbps"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">{t('predict.labels.cholesterol')}</label>
                <input
                  type="number"
                  name="chol"
                  value={formData.chol}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-600 dark:text-slate-300 mb-1">{t('predict.labels.stDepression')}</label>
                <input
                  type="number"
                  step="0.1"
                  name="oldpeak"
            ...
            >
              {loading ? (
                ...
                `${t('common.predict')} (${t('diseases.heart.name')})`
              )}
            </button>
          </form>
        </div>
